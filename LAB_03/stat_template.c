...
void fill_with_randoms(int array[], int n, int a, int b) {
    for (int i = 0; i < n; i++) {
...
void pmf(double v[], int trials) {
    ...
    for (int i = 0; i < trials; i++) {
        int first_dice = (rand() % 6) + 1;
        int second_dice = (rand() % 6) + 1;
        int sum = first_dice + second_dice;
        results[sum] += 1;
    }
    for (int i = 2; i <= 12; i++) {
        v[i] = results[i] / trials;
    }
}
...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
    for (int i = x_start; i < x_start + n; i++) {
        int mark_amount = round(v[i] / y_scale);
        printf("%-2d", i);
        printf(" |");
        for (int j = 0; j < mark_amount; j++) {
            printf("%c", mark);
        }
        printf(" %0.3f\n", v[i]);
    }
...
void monty_hall(int n, int *p_switch_wins) {
    ...
    *p_switch_wins = n - no_switch_wins;
}
